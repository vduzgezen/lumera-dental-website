...
const SortableHeader = ({ 
  ...
  // Inset shadows for the sticky bug bypass
  let focusStyle = "";
  if (isActive) {
  ...
  const sortedCases = useMemo(() => {
    ...
    return [...cases].sort((a, b) => {
      ...
      switch (sortConfig.key) {
        ...
        case "product": aVal = a.product; bVal = b.product; break; // ✅ Changed from restoration to product
        ...
        <table className="w-full text-sm text-left border-collapse min-w-[1000px]">
          <thead className="bg-surface sticky top-0 z-10">
            <tr>
              ...
              {/* ✅ Removed Clinic column from headers */}
              {isDoctor ? (
              ...
              {/* ✅ Renamed Restoration to Product (Units) */}
          ...
          <tbody className="divide-y divide-border">
            ...
            {sortedCases.length === 0 && (
               <tr><td className="p-12 text-center text-muted" colSpan={isDoctor ? 7 : 8}>No cases found.</td></tr>
            )}
          </tbody>
        </table>
      </div>
